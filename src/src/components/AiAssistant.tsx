...
import { FaMagic, FaTimes, FaPaperPlane, FaUserCircle, FaRobot } from 'react-icons/fa';
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
        {messages.length === 0 && (
          <div className="text-center text-slate-500 p-4">
            <FaRobot className="w-12 h-12 mx-auto text-slate-400 mb-2"/>
        ...
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
            {msg.sender === 'ai' && <div className="w-8 h-8 rounded-full bg-secondary text-white flex items-center justify-center flex-shrink-0"><FaRobot className="w-5 h-5"/></div>}
            <div className={`max-w-xs md:max-w-sm p-3 rounded-lg ${msg.sender === 'user' ? 'bg-primary text-white' : 'bg-slate-100 text-slate-800'}`}>
             ...
             {msg.sender === 'user' && <div className="w-8 h-8 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center flex-shrink-0"><FaUserCircle className="w-6 h-6"/></div>}
          </div>
        ))}
        {isLoading && (
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 rounded-full bg-secondary text-white flex items-center justify-center flex-shrink-0"><FaRobot className="w-5 h-5"/></div>
