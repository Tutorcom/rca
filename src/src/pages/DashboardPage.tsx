...
import { FaFileContract, FaUsers, FaCheckCircle, FaClock, FaFileInvoiceDollar, FaFileAlt, FaCalendarAlt } from 'react-icons/fa';
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {user.role === 'admin' ? (
                    <>
                        <StatCard title="Active Projects" value={stats.admin.activeProjects} icon={<FaFileContract className="w-6 h-6"/>} />
                        <StatCard title="Active Clients" value={stats.admin.activeClients} icon={<FaUsers className="w-6 h-6"/>} />
                        <StatCard title="Pending Approvals" value={stats.admin.pendingReviews} icon={<FaClock className="w-6 h-6"/>} />
                        <StatCard title="Tasks Overdue" value={stats.admin.overdueTasks} icon={<FaCheckCircle className="w-6 h-6"/>} />
                    </>
                ) : (
                    <>
                        <StatCard title="My Active Projects" value={stats.contractor.activeProjects} icon={<FaFileContract className="w-6 h-6"/>} />
                        <StatCard title="My Pending Tasks" value={stats.contractor.pendingTasks} icon={<FaCheckCircle className="w-6 h-6"/>} />
                        <StatCard title="Unpaid Invoices" value={stats.contractor.unpaidInvoices} icon={<FaFileInvoiceDollar className="w-6 h-6"/>} />
                        <StatCard title="Completed Projects" value={stats.contractor.completedProjects} icon={<FaFileAlt className="w-6 h-6"/>} />
                    </>
                )}
            </div>
